...
def test_vvb_billader_summerer_begge():
    result = compute_tilgjengelig_kutt_kw(
        strategi=STRATEGI_VVB_BILLADER,
        vvb_power_w=1800.0,
        billader_power_w=3600.0,
    )
    assert result == pytest.approx(5.4)


def test_vvb_billader_uten_billader_aktiv():
    """Billader under 100 W: bidrar ikke."""
    result = compute_tilgjengelig_kutt_kw(
        strategi=STRATEGI_VVB_BILLADER,
        vvb_power_w=2000.0,
        billader_power_w=50.0,
    )
    assert result == pytest.approx(2.0)


def test_vvb_billader_kun_billader():
    """Hvis bare billader er aktiv: returner billader-effekt."""
    result = compute_tilgjengelig_kutt_kw(
        strategi=STRATEGI_VVB_BILLADER,
        vvb_power_w=None,
        billader_power_w=11000.0,
    )
